...
def searchZnf():
    ...
    if form.validate_on_submit():
        ...
        if not znf_item.chip_data:
            return render_template("No_result.html")
        else:
            data_name = znf_item.chip_data[0].data_name
            # return redirect(url_for('/KZFP/zinc_fingure/%s' % id))
            return kzfp_one(data_name)
...
def searchRepeat():
    ...
    if form.validate_on_submit():
        ...
        repeat_item = session.query(DB.Repeat).filter_by(repeat_name=zz).first()
...
def kzfp_one(data_name):
    ...
    if chip_data is None or chip_data.znf is None:
        return render_template("No_result.html")
    znf = chip_data.znf
    znf_data = {}
    ensembl = znf.ensembl
    ...
    znf_data["data_no"] = chip_data.data_name
    znf_data["peaks_num"] = chip_data.peak_number
